...
!!! info "WSO2 Identity Server Compatibility"
    This guide demonstrates JWT Grant configuration using WSO2 Identity Server as an example identity provider. WSO2 API Manager is compatible with WSO2 Identity Server versions 6.1.0 and 7.0.0. While the core configuration steps remain the same across these versions, there may be minor UI differences, particularly if you choose to use WSO2 IS 7.0.0's new Console interface.
...
!!! note
    WSO2 API Manager is compatible with WSO2 Identity Server versions 6.1.0 and 7.0.0. The steps below are tested with WSO2 IS 6.1.0. If you are using WSO2 IS 7.0.0, the UI might differ slightly, but the core configuration remains the same.

1. Download and install WSO2 IS 6.1.0 or 7.0.0 from [WSO2 Identity Server](https://wso2.com/identity-and-access-management/#).
...
3. Sign in to the WSO2 IS Management Console (`https://<IS_Server_Host>:9444/carbon`). 
    ...
    !!! tip "WSO2 IS 7.0.0 Users"
        If you are using WSO2 IS 7.0.0, you can access the new Console at `https://<IS_Server_Host>:9444/console` for a modern UI experience. However, the Management Console (`https://<IS_Server_Host>:9444/carbon`) is still available and the steps below apply to both interfaces.
...
4. Provide the following values to configure the Identity Provider.
    -   **Identity Provider Name :** Enter the issuer name as the identity provider name. This is used to generate the JWT assertion. The issuer of a JWT generated by WSO2 IS is by default `https://localhost:9444/oauth2/token` (this remains consistent across WSO2 IS versions 6.1.0 and 7.0.0).
    -   **Identity Provider Public Certificate :** Choose the radio button named **Upload IDP certificate** so that the field named **Identity Provider Public Certificate** will be enabled. This is the certificate which will be used to sign the JWT assertion.
        ...
        !!! info
            ...
            !!! note
                The certificate export process is the same for both WSO2 IS 6.1.0 and 7.0.0.
...
6. Fill in the **Service Provider Name** and provide a brief **Description** of the service provider. See [Adding a Service Provider](https://is.docs.wso2.com/en/latest/guides/applications/) for more information.
