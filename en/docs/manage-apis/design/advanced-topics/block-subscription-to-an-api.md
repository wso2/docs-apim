...
!!! info
    API level subscription blocking is useful to control only the subscriptions created for a specific API by a user. If you want to block all API requests from a specific application/user/specific IP address or to a specific API, you can use [request denylisting]({{base_path}}/manage-apis/design/rate-limiting/access-control/#denying-requests).
...
1.  Create two APIs.
     ...
     2.  Create two APIs and publish them to WSO2 API Developer Portal.
         ...
         <p>For more information, see [Create and Publish an API]({{base_path}}/manage-apis/design/create-api/create-rest-api/create-a-rest-api/).</p>
         </div>
         </html>
