...
## Deploying and Publishing an API

### Deploy an API

Deploying an API means making it accessible through a gateway. Using the Publisher Portal, you can deploy an API to a specific API Gateway environment. To invoke the API, you must ensure it is published on the Developer Portal and deployed in a Gateway environment. Before deployment, you must create a revision of the API.

When you undeploy an API, it is removed from the API Gateway environment, making it unavailable for use.

### Publish an API

API publishing makes your API available for subscription. When you create an API, its metadata is added to the Developer Portal, but it is not yet deployed to the API Gateway. This means subscribers cannot see or access it. Once you publish the API, it becomes visible in the Developer Portal, allowing users to discover and subscribe to it.
