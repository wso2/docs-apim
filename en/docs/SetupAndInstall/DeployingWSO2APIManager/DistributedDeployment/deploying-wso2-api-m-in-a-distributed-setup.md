...
You will now configure the inter-component relationships of the distributed setup by modifying their `<API-M_HOME>/repository/conf/deployment.toml` files. It is recommended to start the components in the following order: Key Manager, Traffic Manager, Publisher, Dev Portal and Gateway.
...
!!! note
    It is not recommended to share the Solr directory between the Dev Portal and Publisher servers. You need to have separate Solr directories for each of the latter mentioned servers so that they will perform Solr indexing separately.
!!! warning
...
1.  Open the `<API-M_HOME>/repository/conf/deployment.toml` file in the API Publisher node and make the following changes.
    ...
    3.  Configure the Dev portal URL to appear in the Publisher UI.
        ...
        -   [**Dev Portal with HA**](#HA-DevPortal-Publisher)
...
This section involves setting up the Dev Portal node and enabling it to work with the other components in the distributed deployment .

1.  Open the `<API-M_HOME>/repository/conf/deployment.toml` file in the Dev Portal node and make the following changes.

    1.  Configure the **Dev Portal with the Key Manager.**

        -   [**Single Key Manager**](#single-Key-Manager-devPortal)
        -   [**Key Manager with HA**](#HA-Key-Manager-devPortal)

        Configure the **Dev Portal with a single Key Manager** as follows:
        ...
        Configure the **Dev Portal with multiple Key Managers** that are fronted by a load balancer as follows:
    ...
    3.  Configure the **Dev Portal with** the **Gateway** .
        ...
        -   If you are using **multiple Gateway nodes** , configure the **Dev Portal** with the **Gateway nodes** as follows:
            ...
            Configure the **Dev Portal when working with multiple Gateways** that are fronted by a load balancer as follows:
...
3.  Optionally, configure High Availability (HA) for the Dev Portal.
    ...
    Make a copy of the active instance configured above and use this copy as the second Dev Portal active instance.

4.  Start the Dev Portal node(s) by typing the following command in the command prompt. For more information on starting a WSO2 server, see [Starting the server](../../../InstallationGuide/running-the-product/) .
