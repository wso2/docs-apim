...
Google Cloud's [Vertex AI](https://cloud.google.com/vertex-ai/docs) is a platform that hosts foundation models from a range of providers. WSO2 API Manager integrates with Vertex AI through **default AI Service Providers**, so you can expose and govern Vertex AI models through the AI Gateway.

Two Vertex AI providers are available out of the box:

<table>
    <thead>
        <tr>
            <th style="width: 25%">Provider</th>
            <th style="width: 40%">Models hosted</th>
            <th style="width: 35%">Vertex model resource</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td><strong>VertexAI-Anthropic</strong></td>
            <td>Anthropic Claude models on Vertex AI</td>
            <td><code>publishers/anthropic/models</code></td>
        </tr>
        <tr>
            <td><strong>VertexAI-Gemini</strong></td>
            <td>Google Gemini models on Vertex AI</td>
            <td><code>publishers/google/models</code></td>
        </tr>
    </tbody>
</table>

Both providers share the same Google Cloud service-account authentication described under [Authentication](#authentication); they differ only in the models they expose and the Vertex model resource their endpoint URL points at.

Unlike the other default providers, Vertex AI is not accessed with a simple API key &mdash; every request is authenticated with a **Google Cloud service account** using OAuth 2.0, and the Gateway obtains and injects the access token on your behalf.

## Configuring Google Vertex AI

Follow these steps to review and customize a Vertex AI provider in your API Manager environment. The steps are identical for **VertexAI-Anthropic** and **VertexAI-Gemini**; select the one you want to configure.
...
3. Find **VertexAI-Anthropic** or **VertexAI-Gemini** in the list and click on it to edit the configuration.
...
<table>
    <thead>
        <tr>
            ...
            <th style="width: 70%">Default Models</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td><strong>VertexAI-Anthropic</strong></td>
            <td><code>claude-opus-4-6</code>, <code>claude-sonnet-4-6</code>, <code>claude-sonnet-5</code></td>
        </tr>
        <tr>
            <td><strong>VertexAI-Gemini</strong></td>
            <td><code>gemini-2.5-flash-lite</code>, <code>gemini-2.5-flash</code>, <code>gemini-2.5-pro</code></td>
        </tr>
    </tbody>
</table>

- To add available models supported by the selected Vertex AI provider, type the model name and press enter.
...
!!! Note
    Make sure the models you add are available in the Google Cloud region you configure on the AI API endpoint. For Anthropic Claude models, they must also be enabled in Vertex AI Model Garden. See [Step 1: Grant Vertex AI permissions in Google Cloud](#step-1-grant-vertex-ai-permissions-in-google-cloud).
...
4. For **Anthropic Claude** models (`VertexAI-Anthropic`), enable/accept the Claude models you intend to use in [Vertex AI Model Garden](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-claude) &mdash; this is a one-time, project-level step, separate from the service-account role.
...
1. Sign in to the Publisher Portal (`https://<hostname>:9443/publisher`) and open the AI API that uses **VertexAI-Anthropic** or **VertexAI-Gemini** as its AI service provider.
...
!!! Note
    ...
    - The **Region** must match a region where your models are available (and, for Claude, enabled in Model Garden). A regional Vertex AI URL requires the same region in both the host and the `locations` path &mdash; the structured builder keeps them consistent, so prefer it over hand-editing the URL.
