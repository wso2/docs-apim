...
1.  Obtain a token using password grant type and `openid` scope. For more information on token generation with password grant type, see [Password Grant Type]({{base_path}}/Learn/APISecurity/OAuth2/GrantTypes/password-grant/). The format of the curl command and a sample is given below :
