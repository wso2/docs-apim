...
![Single endpoint]({{base_path}}/assets/img/learn/single-endpoint.png)
...
![Dedicated endpoint]({{base_path}}/assets/img/learn/dedicated-endpoint.png)
...
2.  Go to the **Endpoints** tab of the API and replace the host and port of the API endpoint with `{uri.var.host}` and `{uri.var.port} ` respectively, as shown below.
    ...
    ![Dedicated backend definition]({{base_path}}/assets/img/learn/dedicated-backend-def.png)
