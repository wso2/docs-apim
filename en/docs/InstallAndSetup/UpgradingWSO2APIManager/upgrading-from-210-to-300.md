...
!!! note
    If you are using WSO2 Identity Server (WSO2 IS) as a Key Manager, follow the instructions in [Upgrading WSO2 IS as the Key Manager to 5.9.0](../UpgradingWSO2ISAsKeyManager/upgrading-from-is-km-530-to-590.md).
...
3.  To start the migration process, run the respective migration script based on your environment.
    ...
    ??? note "Linux/Mac OS"
        Run the [apim210_to_apim300_gateway_artifact_migrator.sh](../../assets/attachments/InstallAndSetup/apim210_to_apim300_gateway_artifact_migrator.sh) script, as shown below, to migrate from WSO2 API Manager 2.1.0 to 3.0.0. 
        ```
    ...
    ??? note "Windows"
        !!! note "Windows - Super Tenant"
            Run the PowerShell script [apim210_to_apim300_gateway_artifact_migrator.ps1](../../assets/attachments/InstallAndSetup/apim210_to_apim300_gateway_artifact_migrator.ps1) as shown below, to migrate from WSO2 API Manager 2.1.0 to 3.0.0.
        ...
        !!! note "Windows - Tenants"
            Run the PowerShell script [apim210_to_apim300_gateway_artifact_migrator_for_tenants.ps1](../../assets/attachments/InstallAndSetup/apim210_to_apim300_gateway_artifact_migrator_for_tenants.ps1) as shown below, to migrate from WSO2 API Manager 2.1.0 to 3.0.0.
...
7.  Migrate the Publisher access control feature.
    ...
    1.  Download and copy the [org.wso2.carbon.apimgt.migrate.client-3.0.x-1.jar](../../assets/attachments/InstallAndSetup/org.wso2.carbon.apimgt.migrate.client-3.0.x-1.jar) to the `<API-M_3.0.0_HOME>/repository/components/dropins` folder. 
...
!!! info
    As you are upgrading from WSO2 API-M Analytics 2.1.0, in order migrate the configurations required to run WSO2 API-M Analytics for WSO2 API-M 3.0.0 carryout the same instructions as mentioned in [Upgrading from 2.5.0 to 3.0.0 - Step 3 - Optionally, migrate the configurations for WSO2 API-M Analytics](https://apim.docs.wso2.com/en/latest/InstallAndSetup/UpgradingWSO2APIManager/upgrading-from-250-to-300/#step-3-optionally-migrate-the-configurations-for-wso2-api-m-analytics) section.
