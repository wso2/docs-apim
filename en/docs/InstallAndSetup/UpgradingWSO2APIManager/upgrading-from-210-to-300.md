...
5.  Copy the keystores (i.e., `client-truststore.jks`, `wso2cabon.jks` and any other custom JKS) used in the previous version and replace the existing keystores in the `<API-M_3.0.0_HOME>/repository/resources/security` directory.
    ...
    !!! Attention
        In API Manager 3.0.0, it is required to use ceritificate with RSA key size greater than 2048. If you have used a certificate that has weak RSA key (key size less than 2048) in previous version, you need to add following configuration to `<API-M_3.0.0_HOME>/repository/conf/deployment.toml` to configure internal and primary keystores. You should point the internal keystore to the keystore copied from API Manager 2.0.0 and primary keystore can be pointed to a keystore with a ceritificate, which has strong RSA key. 
