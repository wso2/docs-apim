...
Set the `javax.xml.stream.isCoalescing` property to `false` by adding the following configuration in deployment.toml file resides in `<APIM-Home>/repository/conf` directory. 
