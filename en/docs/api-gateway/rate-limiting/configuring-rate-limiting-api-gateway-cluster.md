...
with multiple Gateways. In such scenarios, for [Burst Control]({{base_path}}/api-design-manage/design/rate-limiting/assign-business-plans/#burst-control-spike-arrest) and [Backend Rate Limiting]({{base_path}}/api-design-manage/design/rate-limiting/protect-backend-services) to work properly, it requires maintaining
...
| pool_options.max_wait_millis               | The maximum amount of time, in milliseconds, to wait for a connection to become available from the pool before an exception is thrown. <br/>**For production deployments, configure a suitable maximum wait time based on your deployment requirements.** | Mandatory           | 3000         | -1 (Jedis default) |
...
Once configurations are done, you can refer the docs for [Burst Control]({{base_path}}/api-design-manage/design/rate-limiting/assign-business-plans/#burst-control-spike-arrest) and [Backend Rate Limiting]({{base_path}}/api-design-manage/design/rate-limiting/protect-backend-services) and test the functionality with the Hybrid Mode enabled. 
