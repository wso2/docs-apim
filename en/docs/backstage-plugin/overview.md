...
- Self-hosted gateway discovery (via the `discoveryUrl` endpoint, typically `/rest-apis`) only discovers APIs that were onboarded through the WSO2 API Platform Publisher. APIs created directly as Kubernetes CRDs on the gateway (e.g., via OpenChoreo's `api-management` trait) are not surfaced by the gateway controller's discovery endpoint and are therefore **not discovered** by this plugin. These APIs may still be deployed and operational on the same gateway but remain invisible from the plugin's catalog view. This includes APIs deployed through OpenChoreo's WSO2 API Platform Gateway module when created via the cluster trait mechanism rather than through the WSO2 API Platform Publisher API.
