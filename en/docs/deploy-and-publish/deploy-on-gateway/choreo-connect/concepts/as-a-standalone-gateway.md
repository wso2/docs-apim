...
Also, you can define endpoint configurations globally using the `x-wso2-endpoints` extension as explained in [here]({{base_path}}/reference/vendor-extensions-catalog/#x-wso2-endpointss). Then, endpoints available under the `x-wso2-endpoints` can use with `x-wso2-production-endpoints`. For a detailed example, 
