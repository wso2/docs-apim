...
| maxRouteTimeoutInSeconds | Maximum value accepted as the Endpoint Level Route Timeout. If a larger Route Timeout value is set at the endpoint level using the `timeoutInMillis` configuration, the `maxRouteTimeoutInSeconds` value will override the latter mentioned Endpoint Level Route Timeout value. |
...
- [Retry Policies]({{base_path}}/deploy-and-publish/deploy-on-gateway/choreo-connect/resiliency/retry)