...
Properties listed above are mostly used for debugging, and users can decide the log level according to their needs. However, API Logs allows you to log additional properties that can be used for advanced debugging scenarios. Follow the steps below to configure additional properties.
