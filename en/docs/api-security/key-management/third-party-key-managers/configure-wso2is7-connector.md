...
* Under the **Key Manager Endpoints** section, provide the following values:
    ...
    | UserInfo Endpoint             | <ul><li>`https://localhost:9444/scim2/Me` or <li>`https://localhost:9444/keymanager-operations/user-info` (see update level requirements below). </ul> See [UserInfo Endpoint](#userinfo-endpoint) for more details. |
...
* Under **Connector Configurations**, select the authentication method for communication between API Manager and WSO2 Identity Server 7.x. Two authentication options are available:

    - **Basic Authentication**: Uses username and password credentials.
    - **Mutual SSL**: Uses certificate-based authentication for enhanced security.
...
* Configure the connector settings based on your selected authentication method:

    | Configuration                                           | Value                                                                                                                                                                                                                                                                                                               |
    |---------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | Authentication Type                                     | Select either **Basic Authentication** or **Mutual SSL**.                                                                                                                                                                                                                                                           |
    | Identity Username (for Mutual SSL only)                 | Provide the tenant-qualified username of a user in WSO2 IS 7.x who has permissions to invoke the DCR, Introspection, API Resource Management, and Roles APIs. WSO2 IS 7.x uses this user for authorization when processing related requests.                                                                       |
    ...
    | Username (for Basic Authentication only)                          | `admin`                                                                                                                                                                                                                                                                                                             |
    | Password (for Basic Authentication only)                         | `admin`                                                                                                                                                                                                                                                                                                             |
...
!!! Note
    Using `https://localhost:9444/keymanager-operations/user-info` is supported from the following WSO2 API Manager 4.6.0 update levels onwards:

    - All-in-one distribution (`wso2am`): 4.6.0.47
    - Distributed deployment - Control Plane (`wso2am-acp`): 4.6.0.48
    - Distributed deployment - Universal Gateway (`wso2am-universal-gw`): 4.6.0.46
