...
2.  On the landing page, click on the ‘Create API with AI’ Button at the top of the page or if you have already created an API, click the ‘Create API with AI’ Button on the top left to navigate to API Design Assistant.

3. Send a query to the Assistant in two ways:
- **Select a sample query** by clicking on a sample card.  
- **Enter a custom prompt**, such as the one below. If you want to modify it further, you may enter additional queries as needed. 

    **Create an API for a university**  
