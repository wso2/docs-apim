...
| Mock Implementation      | **Mock implementation** in WSO2 API Manager uses the built-in JavaScript engine of Synapse to mock the responses and can be used per HTTP resource of the API. For more information on the Prototype Implementation, see [Create a Mock API with an Inline Script]({{base_path}}/design/prototype-api/create-a-mock-api-with-an-inline-script/).|
