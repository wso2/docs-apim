...
After an API Product is created, it can be deployed to the selected API Gateway as an independent Synapse API artifact to test it before making it available for consumers. API Product provider can publish the API Product once the testing is completed.
