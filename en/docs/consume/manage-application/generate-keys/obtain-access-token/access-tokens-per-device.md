...
However, this change in behavior is particularly relevant when considering scenarios where the same user is utilizing the same application across multiple devices. For example, if a user has the same application installed on both their iPhone and iPad, and a token has been generated on the iPhone, requesting a token on the iPad within the same validity period could result in obtaining the same token. Consequently, if one of these devices revokes the token, such as during a logout operation, the token obtained for the other device will also become invalid. This is because both devices are utilizing identical tokens.
