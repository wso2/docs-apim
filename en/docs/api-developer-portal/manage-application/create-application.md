...
WSO2 API Manager comes with a pre-created default application, which allows unlimited access by default. Default application creation can be disabled by adding the configuration `"DisableDefaultApplicationCreation" : true` in to the `Advanced Configurations` under `Settings` of the `admin` portal. You can also [create]({{base_path}}/api-developer-portal/manage-application/create-application.md#create-a-new-application) your own.
