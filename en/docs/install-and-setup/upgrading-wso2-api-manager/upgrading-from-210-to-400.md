# Upgrading API Manager from 2.1.0 to 4.0.0

The following information describes how to upgrade your API Manager server **from APIM 2.1.0 to 4.0.0**.
...
If there are frequently updating registry properties, having the versioning enabled for registry resources in the registry can lead to unnecessary growth in the registry related tables in the database. To avoid this, versioning has been disabled in API Manager 4.0.0.
...
5.  If you manually added any custom OSGI bundles to the `<API-M_2.1.0_HOME>/repository/components/dropins` directory, copy those to the `<API-M_4.0.0_HOME>/repository/components/dropins` directory. 

6.  If you manually added any JAR files to the `<API-M_2.1.0_HOME>/repository/components/lib` directory, copy those and paste them in the `<API-M_4.0.0_HOME>/repository/components/lib` directory.
...
3.  Upgrade the WSO2 API Manager database from version 2.1.0 to version 4.0.0 by executing the relevant database script, from the scripts that are provided below, on the `WSO2AM_DB` database.
...
6.  Upgrade the Identity component in WSO2 API Manager from version 5.3.0 to 5.11.0.
    ...
    3.  Open the `migration-config.yaml` file in the migration-resources directory and make sure that the `currentVersion` element is set to 5.3.0, as shown below.
        ...
        !!! note
            Make sure you have enabled migration by setting the `migrationEnable` element to `true` as shown above. You have to remove the following 4 steps from  migration-config.yaml.
                ```
                -
...
7.  Migrate the API Manager artifacts.
    ...
    3. Prior to API-M migration, run the below command to execute the pre-migration step that will validate your old data.
        ...
        ``` tab="Linux / Mac OS"
        sh api-manager.sh -DmigrateFromVersion=2.1.0 -DrunPreMigration
        ```

        ``` tab="Windows"
        api-manager.bat -DmigrateFromVersion='2.1.0' -DrunPreMigration
        ```
        ...
        !!! note "If you want to save the invalid API definitions"
            ...
            ```tab="Linux / Mac OS"
            sh api-manager.sh -DmigrateFromVersion=2.1.0 -DrunPreMigration -DsaveInvalidDefinition
            ```

            ``` tab="Windows"
            api-manager.bat -DmigrateFromVersion='2.1.0' -DrunPreMigration -DsaveInvalidDefinition
            ```
...
8.  Re-index the artifacts in the registry.
    ...
    6.  Stop the WSO2 API-M server and remove the `tenantloader-1.0.jar` from the `<API-M_4.0.0_HOME>/repository/components/dropins` directory.
        ...
        !!! note
            ...
            Make sure you have copied the tenant's userstores if you have configured them in WSO2 API Manager 2.1.0.
