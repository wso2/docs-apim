...
The following information describes how to upgrade your API Manager server **from APIM 2.6.0 to 4.1.0**.
...
2.  Open the `<API-M_4.1.0_HOME>/repository/conf/deployment.toml` file and provide the datasource configurations for the following databases.
    ...
    !!! note
        ...
        It is recommended to use the default H2 database for the `WSO2_MB_STORE_DB` database in API-Manager. So do **not** migrate `WSO2_MB_STORE_DB` database from API-M 2.1.0 version to API-M 4.1.0 version, and use the **default H2** `WSO2_MB_STORE_DB` database available in API-M 4.1.0 version.
...
5. If you have enabled any other feature related configurations in API Manager 2.6.0, make sure to add them in to `<API-M_4.1.0_HOME>/repository/conf/deployment.toml` file.
...
6. If you already have used secondary user stores in API-M 3.2.0, you have to copy the user store file created inside the old API-M version to the new APIM version.
...
5.  Shutdown the API-M server.
    ...
    !!! note
        ...
        Make sure you have copied the tenants userstores if you have configured them in API-M 2.6.0
