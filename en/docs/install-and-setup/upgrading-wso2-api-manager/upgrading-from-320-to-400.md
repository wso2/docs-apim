...
!!! note "NOTE"
    Alternatively, it is possible to turn on registry versioning in API Manager 4.0.0 and continue. But this is highly **NOT RECOMMENDED** and these configurations should only be changed once.
...
6. If you already have used secondary user stores in API-M 3.2.0, you have to copy the user store file created inside the old API-M version to the new API-M version.
    ...
    - For secondary user stores created for tenants,
        ...
        You have to copy the userstores to respective tenant directories after executing step 5 as the tenant directories (`<API-M_3.2.0_HOME>/repository/tenants/`) at API-M 4.0.0 are populated after migrating the api-m components.
