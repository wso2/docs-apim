# Upgrading API Manager from 2.6.0 to 4.0.0
...
2.  If you manually added any custom OSGI bundles to the `<API-M_2.6.0_HOME>/repository/components/dropins` directory, copy those to the `<API-M_4.0.0_HOME>/repository/components/dropins` directory. 

3.  If you manually added any JAR files to the `<API-M_2.6.0_HOME>/repository/components/lib` directory, copy those and paste them in the `<API-M_4.0.0_HOME>/repository/components/lib` directory.
...
2.  Upgrade the WSO2 API Manager database from version 2.6.0 to version 4.0.0 by executing the relevant database script, from the scripts that are provided below, on the `WSO2AM_DB` database.
    ...
    ??? info "DB Scripts"
        ...
        CREATE TABLE IF NOT EXISTS IDN_UMA_RESOURCE_SCOPE (
            ...
            SCOPE_NAME VARCHAR(255),
            PRIMARY KEY (ID),
            FOREIGN KEY (RESOURCE_IDENTITY) REFERENCES IDN_UMA_RESOURCE (ID) ON DELETE CASCADEEY (RESOURCE_IDENTITY) REFERENCES IDN_UMA_RESOURCE (ID) ON DELETE CASCADE
        );
...
3.  Open the `migration-config.yaml` file in the migration-resources directory and make sure that the `currentVersion` element is set to 5.7.0, as shown below. Make sure you have enabled migration by setting the `migrationEnable` element to `true`.
...
3. Prior to API-M migration, run the below command to execute the pre-migration step that will validate your old data.
    ...
    ``` tab="Linux / Mac OS"
    sh api-manager.sh -DmigrateFromVersion=2.6.0 -DrunPreMigration
    ```

    ``` tab="Windows"
    api-manager.bat -DmigrateFromVersion='2.6.0' -DrunPreMigration
    ```
    ...
    !!! note "If you want to save the invalid API definitions"
        ...
        ```tab="Linux / Mac OS"
        sh api-manager.sh -DmigrateFromVersion=2.6.0 -DrunPreMigration -DsaveInvalidDefinition
        ```

        ``` tab="Windows"
        api-manager.bat -DmigrateFromVersion='2.6.0' -DrunPreMigration -DsaveInvalidDefinition
        ```
...
4.  Start the API-M server as follows.
    ...
    ``` tab="Linux / Mac OS"
    sh api-manager.sh -DmigrateFromVersion=2.6.0
    ```

    ``` tab="Windows"
    api-manager.bat -DmigrateFromVersion=2.6.0
    ```
