# Upgrading API Manager from 2.2.0 to 3.2.0

The following information describes how to upgrade your API Manager server **from APIM 2.2.0 to 3.2.0**.
...
Follow the instructions below to upgrade your WSO2 API Manager server **from WSO2 API-M 2.2.0 to 3.2.0**.
...
3.  To start the migration process, run the respective migration script based on your environment.
    ...
    ??? note "Linux/Mac OS"
        Run the [apim220_to_apim320_gateway_artifact_migrator.sh]({{base_path}}/assets/attachments/install-and-setup/apim220_to_apim320_gateway_artifact_migrator.sh) script, as shown below, to migrate from WSO2 API Manager 2.2.0 to 3.2.0. 
        ```
        ./apim220_to_apim320_gateway_artifact_migrator.sh <API-definitions-path>
        ```
        
        !!! note
            If you are getting a "Permission Denied" message when you execute the above command, grant the permission as follows.
            ```
            chmod 777 apim220_to_apim320_gateway_artifact_migrator.sh
            ```
    ...
    ??? note "Windows"
        !!! note "Windows - Super Tenant"
            Run the PowerShell script [apim220_to_apim320_gateway_artifact_migrator.ps1]({{base_path}}/assets/attachments/install-and-setup/apim220_to_apim320_gateway_artifact_migrator.ps1) as shown below, to migrate from WSO2 API Manager 2.2.0 to 3.2.0.
        ...
        !!! note "Windows - Tenants"
            Run the PowerShell script [apim220_to_apim320_gateway_artifact_migrator_for_tenants.ps1]({{base_path}}/assets/attachments/install-and-setup/apim220_to_apim320_gateway_artifact_migrator_for_tenants.ps1) as shown below, to migrate from WSO2 API Manager 2.2.0 to 3.2.0.
        ...
        !!! info "Troubleshooting"
            ...
            **Why do I get the following error - `apim220_to_apim320_gateway_artifact_migrator.ps1`/`apim220_to_apim320_gateway_artifact_migrator_for_tenants.ps1` cannot be loaded because the execution of scripts is disabled on this system?**

            When running the `apim220_to_apim320_gateway_artifact_migrator.ps1` script, if the execution process is aborted with the above error, it means that the execution of unknown scripts is disabled in the system.
...
4.  Upgrade the WSO2 API Manager database from version 2.2.0 to version 3.2.0 by executing the relevant database script, from the scripts that are provided below, on the `WSO2AM_DB` database.
...
7.  Migrate the API Manager artifacts.
    ...
    3.  Start the API-M server as follows.
        ...
        ``` tab="Linux / Mac OS"
        sh wso2server.sh -DignoreCrossTenantSubscriptions=true -DmigrateFromVersion=2.2.0
        ```
    
        ``` tab="Windows"
        wso2server.bat -DignoreCrossTenantSubscriptions=true -DmigrateFromVersion=2.2.0
        ```
