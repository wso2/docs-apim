# Upgrading API Manager from 3.0.0 to 4.0.0
...
2.   If you have used a separate DB for user management, you need to update `<API-M_4.0.0_HOME>/repository/conf/deployment.toml` file as follows, to point to the correct database for user management purposes.
...
2. If you manually added any custom OSGI bundles to the `<API-M_3.0.0_HOME>/repository/components/dropins` directory, copy those to the `<API-M_4.0.0_HOME>/repository/components/dropins` directory. 

3.  If you manually added any JAR files to the `<API-M_3.0.0_HOME>/repository/components/lib` directory, copy those and paste them in the `<API-M_4.0.0_HOME>/repository/components/lib` directory.
...
6. If you already have used secondary user stores in API-M 3.0.0, you have to copy the user store file created inside the old API-M version to the new API-M version.
    ...
    - For secondary user stores created for the super tenant,
        ...
        You have to copy the `<API-M_3.0.0_HOME>/repository/deployment/server/userstores/` directory inside `<API-M_4.0.0_HOME>/repository/deployment/server/` directory.

    - For secondary user stores created for tenants,

        You have to copy the userstores to respective tenant directories after executing [step 5](#step-5-migrate-api-manager-components) as the tenant directories (`<API-M_3.0.0_HOME>/repository/tenants/`) at API-M 4.0.0 are populated after migrating the api-m components.

### Step 3 - Migrate API-M Database

1.  Make sure you backed up all the databases.

2.  Upgrade the WSO2 API Manager database from version 3.0.0 to version 4.0.0 by executing the relevant database script, from the scripts that are provided below, on the `WSO2AM_DB` database.
...
4. Make sure you have enabled migration by setting the `migrationEnable` element to `true` as shown above. You have to remove the following 3 steps from  migration-config.yaml which is included under version: "5.10.0".
    ```
    -
...
3. Prior to API-M migration, run the below command to execute the pre-migration step that will validate your old data.
    ...
    ``` tab="Linux / Mac OS"
    sh api-manager.sh -DmigrateFromVersion=3.0.0 -DrunPreMigration
    ```

    ``` tab="Windows"
    api-manager.bat -DmigrateFromVersion='3.0.0' -DrunPreMigration
    ```
    ...
    !!! note "If you want to save the invalid API definitions"
        ...
        ```tab="Linux / Mac OS"
        sh api-manager.sh -DmigrateFromVersion=3.0.0 -DrunPreMigration -DsaveInvalidDefinition
        ```

        ``` tab="Windows"
        api-manager.bat -DmigrateFromVersion='3.0.0' -DrunPreMigration -DsaveInvalidDefinition
        ```
...
5. Shutdown the API-M server.
   ...
   -   Remove the `org.wso2.carbon.apimgt.migrate.client-4.0.0.4.jar` file, which is in the `<API-M_4.0.0_HOME>/repository/components/dropins` directory.
