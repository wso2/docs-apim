...
At server startup, the Gateway fetches all the subscription validation metadata such as APIs, API policies, URL templates, applications, application keys, application policies, subscriptions, and subscription policies from the database and persists them in memory.
Therefore, when required to validate a token, the Universal Gateway uses the in-memory data store to validate token details. For more information, see [Key Manager]({{base_path}}/get-started/apim-architecture/#key-manager).
...
- For more information, see [Integration with a third-party OAuth Authorization Server]({{base_path}}/administer/key-managers/overview/)
