...
| APPLICATION_STATUS | The current approval status of the application (for example, APPROVED, REJECTED, ON_HOLD), controlled by the application creation workflow. |
...
| Column | Description |
|--------|-------------|
| UUID | Auto-generated unique identifier for this key mapping record. |
...
| STATE | The current lifecycle state of the key (for example, CREATED, APPROVED, REJECTED). |
