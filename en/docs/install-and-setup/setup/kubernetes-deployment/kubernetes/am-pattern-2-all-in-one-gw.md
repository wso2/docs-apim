...
2. Open `values-aio.yaml` and update the two sections below before deploying.
    ...
    **Database connection** — point to the database you set up in Step 6:
    ...
    === "Externally managed database"
        ...
        Replace the hostname with your managed database endpoint. Most managed database services (e.g. RDS, Cloud SQL) require SSL:
...
4. Wait for the pod to be ready:
    ...
    The All-in-One pod should show `1/1 Running` before deploying the Gateway.
...
4. Wait for the Gateway pods to be ready:
    ...
    Both Gateway pods should show `1/1 Running`. If pods are `OOMKilled`, your cluster may not have enough resources — see the [Resource Requirements](kubernetes-overview.md#resource-requirements) section. If a pod is restarting for other reasons, check for errors:
