...
4. Wait for the ACP pods to be ready:
    ...
    The ACP pods should show `1/1 Running` before deploying the Traffic Manager.
...
By default, database passwords and other sensitive values are stored as plain text in the values files. This is acceptable for local testing but a security risk in production. Use the WSO2 cipher tool or `apictl` to encrypt these values before deploying.
