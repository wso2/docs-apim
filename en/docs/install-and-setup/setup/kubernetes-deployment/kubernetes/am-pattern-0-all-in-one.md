...
By default, database passwords and other sensitive values are stored as plain text in `values.yaml`. This is acceptable for local testing but a security risk in production — anyone with access to the file can read the credentials. Use the WSO2 cipher tool or `apictl` to encrypt these values before deploying.
