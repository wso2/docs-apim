...
!!! Note "Graceful shutdown behavior of the PassThrough transport"
    By default, when an API Manager Gateway node begins a graceful shutdown, it continues to be treated as available for some time before actually shutting down. This is because the graceful shutdown timeout of the PassThrough transport is governed by the `socket timeout` value, and the node remains available until this period elapses. To shorten this window, reduce the socket_timeout value under `[transport.http]` and `[transport.https]` in the `<API-M_HOME>/repository/conf/deployment.toml` file to a lower value instead of the default of 180000ms. Note that this value should still be set high enough to comfortably accommodate the response time of any in-flight requests, so real traffic will not be cut off prematurely.
