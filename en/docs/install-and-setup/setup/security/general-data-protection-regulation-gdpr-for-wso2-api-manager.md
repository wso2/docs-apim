...
This is the master configuration file. You can configure this file depending on the metadata database tables, access logs, audit logs, or any other log files on which you want the Identity Anonymization tool to run. The following is a sample `config.json` file:
...
1.  Open the `forgetme.sh` file found inside the `<API-M_HOME>/bin` folder. 
...
!!! note
    ...
    </br>For example, if you have User, REG, and AM databases pointed to MySQL, but your Carbon DB is in H2, then too you can get a DB lock error when running in online mode.
