...
!!! note
    Wildcard certificates are supported to match **any single domain component name only**. Component fragments not being validated is a limitation. For example, `*.a.com` is matched with `name.a.com`. But `n*.a.com` is not supported as a valid component name.
