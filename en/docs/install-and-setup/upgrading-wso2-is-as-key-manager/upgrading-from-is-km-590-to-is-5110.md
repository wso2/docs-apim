...
5. Migrate IS KM 5.9.0 configurations as per the instructions in [Migrating the configurations](https://is.docs.wso2.com/en/5.11.0/setup/migrating-preparing-for-migration/#migrating-the-configurations).
...
9. Modify `create_admin_account` to false in `<IS_HOME>/repository/conf/deployment.toml` if it is not changed already.
...
10. Start WSO2 IS 5.11.0 as follows to carry out the complete Identity component migration.
...
Follow the steps mentioned in [Upgrading API-M from 3.0.0 to 4.1.0]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/300-to-410/upgrading-from-300-to-410/) to upgrade your API-M environment from 3.0.0 to 4.1.0.
...
!!! important
    ...
    - After configuring WSO2 IS as the **Resident Key Manager** and before starting the API-M 4.1.0 server for the first time in **Step 5** under [Step 2 - Upgrade API Manager to 4.1.0]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/upgrading-from-300-to-410/#step-2-upgrade-api-manager-to-410), make sure you have already started WSO2 IS 5.11.0.
