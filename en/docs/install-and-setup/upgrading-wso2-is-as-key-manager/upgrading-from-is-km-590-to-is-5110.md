...
The following information describes how to upgrade your **WSO2 API Manager (WSO2 API-M)** environment **from APIM 3.0.0 to 4.0.0** when **WSO2 Identity Server (WSO2 IS)** is the **Key Manager** in the pre-migrated setup.
...
-   [Step 1 - Upgrade IS as Key Manager 5.9.0 to IS 5.11.0](#step-1-upgrade-is-as-key-manager-590-to-is-5110)
-   [Step 2 - Upgrade API Manager 3.0.0 to 4.0.0](#step-2-upgrade-api-manager-300-to-400)

## Step 1 - Upgrade IS as Key Manager 5.9.0 to IS 5.11.0

- [Step A - Setup IS 5.11.0 as a Key Manager](#step-a-setup-is-5110-as-a-key-manager)
- [Step B - Migrate IS from 5.9.0 to 5.11.0](#step-b-migrate-is-from-590-to-5110)
...
### Step B - Migrate IS from 5.9.0 to 5.11.0

1. Follow Step 2 and 3 under [Step 2 - Upgrade API Manager to 4.0.0]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/upgrading-from-300-to-400/#step-2-upgrade-api-manager-to-400) to backup and upgrade the WSO2 API-M `WSO2AM_DB` from 3.0.0 to 4.0.0. This will be used as the `identity_db` in IS 5.11.0.

2. Follow the guidelines in [WSO2 IS 5.11.0 migration guide](https://is.docs.wso2.com/en/5.11.0/setup/migrating-to-5110/) to migrate your current IS as KM 5.9.0 distribution to IS 5.11.0. Make sure to use the migration resources downloaded from the [latest release](https://github.com/wso2-extensions/apim-identity-migration-resources/releases) under Assets instead of the migration resources found under Step 9 a. in the WSO2 IS 5.11.0 migration guide.
...
## Step 2 - Upgrade API Manager 3.0.0 to 4.0.0

Follow the steps mentioned in [Upgrading API-M from 3.0.0 to 4.0.0]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/upgrading-from-300-to-400/) to upgrade your API-M environment from 3.0.0 to 4.0.0.

!!! important

    -   When following guidelines under [Step 1 - Migrate the API Manager configurations]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/upgrading-from-300-to-400/#step-1-migrate-the-api-manager-configurations), make sure to add
    ...
    -   When following guidelines under [Step 2 - Upgrade API Manager to 4.0.0]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/upgrading-from-300-to-400/#step-2-upgrade-api-manager-to-400), make sure you **SKIP** the following
    steps.

        -   **Step 3** to upgrade the `WSO2AM_DB` from 3.0.0 to 4.0.0. You have already done this in Step 1 of [Step A - Setup IS 5.11.0 as a Key Manager]({{base_path}}/install-and-setup/upgrading-wso2-is-as-key-manager/upgrading-from-is-km-590-to-is-5110/#step-a-setup-is-5110-as-a-key-manager).
        -   **Step 5** to upgrade identity component in API-M from 5.9.0 to 5.11.0. You have already done this in Step 2 of [Step A - Setup IS 5.11.0 as a Key Manager]({{base_path}}/install-and-setup/upgrading-wso2-is-as-key-manager/upgrading-from-is-km-590-to-is-5110/#step-a-setup-is-5110-as-a-key-manager).

    - After configuring WSO2 IS as the **Resident Key Manager** and before starting the API-M 4.0.0 server for the first time in **Step 6** under [Step 2 - Upgrade API Manager to 4.0.0]({{base_path}}/install-and-setup/upgrading-wso2-api-manager/upgrading-from-300-to-400/#step-2-upgrade-api-manager-to-400), make sure you have already started WSO2 IS 5.11.0.
